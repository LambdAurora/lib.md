...
export const Tag = Object.freeze({
    ...
    cite: make_tag('cite'),
    code: make_tag('code', { interpret_inside: false }),
    ...
    s: make_tag('s'),
    samp: make_tag('samp'),
    script: make_tag('script', { interpret_inside: false }),
        ...
        if (this.tag.self_closing) {
            ...
            if (this.children.length !== 0) {
                ...
                for (const child of this.children) {
                    ...
                    result += child.html(!this.tag.interpret_inside);
                }
            }
...
function parse_text(source, is_code_block = false) {
    let end = 0;
    for (let i = 0; i < source.length; i++) {
        ...
        if (character === '<') { // Possible control character.
            ...
            if (is_code_block) { // In the case of a code block we can ignore control characters, except the code ending tag.
                let found = true;

                for (let code_i = 0; code_i < CODE_TAG_END.length; code_i++) {
                    if (i + code_i >= source.length) {
                        found = false;
                        break;
                    } else if (source[i + code_i] !== CODE_TAG_END[i]) {
                        found = false;
                        break;
                    }
                }
    ...
    if (end !== 0) {
        return {
            text: new Text(source.substr(0, end + 1)),
            end: end
        };
    } else {
        return null;
...
function parse_html(source, parent) {
    ...
    while (i < source.length) {
        ...
        if (source[i] === '<') {
            ...
            let text_result = parse_text(sub_source, !parent.tag.interpret_inside);
