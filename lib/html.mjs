...
export class Element extends Node {
	 ...
	 * @return {Element|undefined} the found element if the search is successful, otherwise `undefined`
	 * @since 1.4.2
	 */
		...
		if (result) {
			...
			for (const child of this.children) {
				if (child instanceof Element) {
					const child_result = this.find_element_by_tag_name(child);
...
function parse_text(source, start, parent) {
	...
	for (let i = start; i < source.length; i++) {
		...
		if (character === TAG_START_CODE_POINT) { // Possible control character.
			...
			if (!parent.tag.parse_inside) { // In the case of a code block we can ignore control characters, except the code ending tag.
				const result = TAG_END_REGEX.exec(source.substr(i));
	...
	if (end !== -1) {
		...
		if (length !== 1) end++;
		return {
			text: new Text(source.substr(start, length), TextMode.RAW),
			end: Math.max(1, end)
		};
	} else {
		return null;
...
function parse_html(source, parent) {
	...
	while (i < source.length) {
		if (source.codePointAt(i) === TAG_START_CODE_POINT) {
			const sub_source = source.substr(i);
...
function parse_tag_start(source) {
	...
	const attributes_length = parse_attributes(source.substr(tag_result[0].length), node);
