...
export class Emoji extends Text {
    ...
    toJSON() {
        let json = { type: "emoji", content: this.content };
...
export class InlineCode extends Text {
    ...
    as_html() {
        return html.create_element("code").with_child(this.content);
    }
}
...
export class List extends BlockElement {
    ...
    toString() {
        let entries = this.nodes.map(node => node.toString())
            .map((node, index) => {
                if (this.ordered) {
                    return `${index + this.ordered_start}. ${node}`;
                } else {
                    return `- ${node}`;
                }
            })
...
class TableAlignment {
    constructor(name, right, left, style_table_data) {
        this.name = name;
        this.left = left;
        this.right = right;
...
export const TableAlignments = Object.freeze({
...
export class Table extends BlockElement {
    ...
    toString() {
        let head_columns = this.get_head().nodes.map(column => column.toString());
        let alignments = "| " + this.alignments
                .map((alignment, index) => alignment.to_pretty_string(head_columns[index] ? head_columns.length : 5))
                .join(" | ")
            + " |";
    ...
    toJSON() {
        return { type: "table", rows: this.nodes, alignments: this.alignments.map(alignment => alignment.name) };
    }
}
