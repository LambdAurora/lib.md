...
function to_string_with_linebreaks(nodes) {
    return nodes.map((node, index) => node.toString() + ((index + 1 < nodes.length && !(node instanceof LinebreakElement)) ? " " : "")).join("");
}
...
function purge_linebreaks(nodes) {
    return nodes.filter(node => !(node instanceof LinebreakElement));
}
...
class LinebreakElement extends Text {
    constructor() {
        super("  \n");
    }
...
export const LINEBREAK = new LinebreakElement();
...
export class Document {
    ...
    toString() {
        ...
        return this.blocks.map(block => block.toString() + "\n").join("\n") + "\n"
            + references.map(ref => `[${ref.name}]: ${ref.ref.toString()}`).filter((v, i, arr) => arr.indexOf(v) === i).join("\n") + "\n";
    }
}
