...
export class ListEntry extends Element {
    /**
     * @param {string|Element[]} nodes The content.
     * @param {List} sublist If present, the sublist of this entry, else null.
     */
    constructor(nodes, sublist = null) {
        super(nodes, false);
    ...
    toJSON() {
        return { type: "list_entry", nodes: this.nodes, sublist: this.sublist };
    }
}

const INLINE_HTML_SCRIPT_REGEX = /\<script.*\>.*?\<\/script\>/;
const INLINE_HTML_LINK_REGEX = /\<link.*?\>/;
...
export class InlineHTML extends Element {
    ...
    purge_content() {
        this.content = this.content.replace(INLINE_HTML_SCRIPT_REGEX, "");
        this.content = this.content.replace(INLINE_HTML_LINK_REGEX, "");
    ...
    toJSON() {
        return { type: "inline_html", content: this.content };
    }
}
