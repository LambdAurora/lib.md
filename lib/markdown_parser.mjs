...
function try_parse_link(input, start) {
	...
	if (input[i] === "(") {
		...
		let part = input.substring(i + 1, j - 1).split(" ");
		let url = part.shift();
		...
		let reference = input.substring(i + 1, j);
...
function try_parse_url(input, start) {
	...
	let scheme = input.substring(start, scheme_separator);
...
function try_parse_tag(text, start, delimiter, delimiter_repeat) {
	...
	let tag = rest.substring(delimiter_repeat, i - 1);
		...
		for (let index = 0; index < lines.length; index++) {
			...
			if (options.code_block_from_indent && (found = line.match(CODE_BLOCK_INDENT_DETECTION_REGEX))
				...
				if (end) {
					...
					let remaining = line.substring(end.index + "-->".length);
					push_group();
