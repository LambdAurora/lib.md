...
import { merge_objects, purge_inline_html, is_whitespace } from "./utils.mjs";
...
function try_parse_link(input, start) {
	...
	function get_title() {
		return input.substr(start + 1, i - 2 - start);
	}
	...
	if (input[i] === "(") {
		...
		let part = input.substr(i + 1, (j - i) - 2).split(" ");
		let url = part.shift();
		...
		let reference = input.substr(i + 1, j - i - 1);
...
function try_parse_url(input, start) {
	...
	for (scheme_separator = start; scheme_separator < input.length; scheme_separator++) {
		let char = input[scheme_separator];
		if (char === ":") {
			break;
	...
	let scheme = input.substr(start, scheme_separator - start);

	let next_index = scheme_separator + 1;
	let authority = "";
	let has_slashes = input.indexOf("//", next_index) === next_index;
	...
	if (scheme === "file" && !has_slashes) {
		...
		if (path_separator !== -1) {
			authority = input.substr(scheme_separator + 3, path_separator - next_index);
...
function try_parse_emoji(text, start) {
	let rest = text.substr(start);
	let matched = rest.match(EMOJI_REGEX);
	if (!matched)
		return null;
...
class MarkdownParser {
	...
	group_blocks(string, options = {}) {
		...
		for (let index = 0; index < lines.length; index++) {
			...
			if (options.code_block_from_indent && (found = line.match(CODE_BLOCK_INDENT_DETECTION_REGEX))
				...
				let end = line.match(COMMENT_END_REGEX);
				...
				if (end) {
					if (!found) {
						current += "\n" + line.substr(0, end.index);
					} else {
						current = line.substr(0, end.index);
					}

					let remaining = line.substr(end.index + "-->".length);
					push_group();

					if (remaining.length !== 0) {
						parse_line(remaining, index);
					}
	...
	parse_block(block, options = {}) {
		...
		switch (block.type) {
			...
			case "quote":
				// Quotes
				return new md.BlockQuote(this.parse_blocks(
			...
			case "table":
				...
				for (const index in rows) {
					...
					if (index == 1) {
						...
						let table_row = new md.TableRow(table);

						table_row.nodes = raw_entries.map(entry => {
							return new md.TableEntry(table_row, this.parse_nodes(entry.trim(), false, options));
						});
			...
			default:
				return new md.Paragraph(this.parse_nodes(block.block, true, options));
		}
	}
		...
		while (index < line.length) {
			...
			if (char === "\\" && options.allow_escape) {
				...
				((result = line.substr(index).match(INLINE_HTML_BR_REGEX))
					...
					|| (result = html.parse_comment(line.substr(index))))) {
				if (result.comment) {
				...
				let title = this.parse_nodes(result.title, false, options);
				...
				nodes.push(new md.Spoiler(this.parse_nodes(result.tag, false, options)));
				...
				nodes.push(new md.Strikethrough(this.parse_nodes(result.tag, true, options)));
				...
				const content = this.parse_nodes(result.tag, true, options);
				...
				nodes.push(new md.Highlight(this.parse_nodes(result.tag, true, options)));
