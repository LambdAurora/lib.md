...
class MarkdownParser {
	...
	parse_block(block, options = {}) {
		...
		switch (block.type) {
			...
			case "table": {
				...
				if (rows.length < 2) {
					const nodes = this.parse_nodes(block.block, true, options);
	...
	parse_blocks(string, options = {}) {
		...
		const blocks = this.group_blocks(string, options);
	...
	parse_nodes(line, allow_linebreak, options = {}) {
		...
		while (index < line.length) {
			...
			if (char === "\\" && options.allow_escape) {
				...
				const title = this.parse_nodes(result.title, false, options);
