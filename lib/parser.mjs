...
const INLINE_HTML_DETECTION_REGEX = /^\s*\<.+\>/;
...
class MarkdownParser {
    parse(string) {
        ...
        this.parse_blocks(string).forEach(block => doc.push(block));
    ...
    group_blocks(string) {
        ...
        lines.forEach(line => {
            if (line.match(INLINE_HTML_DETECTION_REGEX) || current_block === "inline_html") {
                if (line === "") {
                    current_block = "none";
                    if (current) {
                        blocks.push(current);
            ...
            } else if ((found = line.match(LIST_DETECTION_REGEX)) || (current_block.startsWith("list") && line.startsWith(" "))) {
                // List
    ...
    parse_blocks(string) {
        let blocks = this.group_blocks(string);
        ...
        return blocks.map(block => {
            ...
            if (block.startsWith("#")) {
            ...
            } else if ((found = block.match(LIST_DETECTION_REGEX))) {
                // List
                ...
                let current = lines.shift().replace(regex, "");
                let raw_list = [];
                lines.forEach(line => {
                    found = line.match(LIST_DETECTION_REGEX);
                    if (found) {
                        let spaces = line.match(/^ {0,1}/);
                        if (spaces) {
                            if (current) {
