...
function try_parse_emoji(text, start) {
    ...
    return { el: new md.Emoji(matched[1]), skip: matched[0].length };
}
    ...
    parse_nodes(line, allow_linebreak, options = {}) {
        ...
        while (index < line.length) {
            ...
            if (char === "\\" && options.allow_escape) {
                ...
                } else if (result[1] === "br" || result[2] === "br") {
                    word = push_text_if_present(word, nodes);
                    if (allow_linebreak) {
            ...
            } else if (char === ":" && options.emoji && (result = try_parse_emoji(line, index))) {
                word = push_text_if_present(word, nodes);
                nodes.push(result.el);
                index += result.skip;
                continue;
