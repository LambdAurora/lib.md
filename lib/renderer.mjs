...
function render_latex(node, options) {
    ...
    let template = html.create_element('template');
...
function render_inline(markdown, nodes, options, allow_linebreak = false) {
    return nodes.map(node => {
        if (node instanceof md.InlineCode) {
            ...
            return render_simple(markdown, node, options, 'strong', allow_linebreak);
            ...
            let element = render_simple(markdown, node, options, 'span', allow_linebreak);
            element.attr('class', options.highlight.class_name);
            return element;
    ...
    blocks.forEach(block => {
        if (block instanceof md.Heading) {
            ...
            /*if (options.inline_html) {
                parent.innerHTML += render_inline(markdown, block.nodes, merge_objects(options, { inline_html_block: true }), true).map(node => {
                    if (typeof node === 'string') {
                        return node;
                    } else {
            ...
            /*if (element.nodeName === '#text' && !options.paragraph_as_text) {
                let paragraph = html.createElement('p');
...
export default function render(markdown, html_doc, options = {}) {
    ...
    doc_div.innerHTML = element.html();
