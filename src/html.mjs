...
export class Element extends Node {
	...
	purge_empty_children() {
		...
		this.children = this.children.filter(child => {
			if (child instanceof Text) {
				// We don't want to filter you simple single whitespaces as it could be: <a>Something</a> <code>hey</code>
	...
	/**
	 * Gets the first occurence of an element that matches.
	 *
	 * @param {(child: Element) => boolean} matcher the matcher function
	 * @returns {Element|undefined} the found element if the search is successful, or `undefined` otherwise
	 * @since 1.6.0
	 */
	get_element_by_match(matcher) {
		return this.children.find(child => child instanceof Element && matcher(child));
	}
	...
	/**
	 * Finds the first occurence of an element that matches.
	 * This method is recursive.
	 *
	 * @param {(element: Element) => boolean} matcher the matcher function
	 * @returns {Element|undefined} the found element if the search is successful, or `undefined` otherwise
	 * @since 1.6.0
	 */
	find_element(matcher) {
	...
	/**
	 * Finds the first occurence of an element by its id.
	 *
	...
	find_element_by_id(name) {
		return this.find_element(child => {
			const id_attr = child.get_attr("id");

			return id_attr && id_attr.value() === name;
		});
	}
	...
	html(options) {
		...
		if (this.tag.self_closing) {
			...
			if (options.prettified && (!this.tag.inline || has_non_inline(this))) {
				result += "\n";
			}
		...
		if (this.children.length !== 0) {
			...
			for (let i = 0; i < this.children.length; i++) {
				...
				if (options.prettified && !options.raw) {
					...
					} else if (last instanceof Text && child instanceof Element) {
...
const DEFAULT_RENDER_OPTIONS = {
	...
	is_prettified: function() {
		return this.prettified && (!this.parent || (this.parent.tag !== Tag.pre && !(this.parent.tag.inline && !has_non_inline(this.parent))));
	},
	is_self_prettified: function(current) {
		return this.prettified && current.tag !== Tag.pre && !(current.tag.inline && !has_non_inline(current));
	},
...
function has_non_inline(el) {
	return el.children.find(node => {
		if (node instanceof Element) {
			if (!node.tag.inline) {
				return true;
			} else {
				return has_non_inline(node);
			}
		}
