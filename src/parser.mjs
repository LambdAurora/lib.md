	...
	of_number_literal: (options) => {
		...
		return (input) => {
			if (input.startsWith("0b"))
				return { length: TokenMatcher.match_binary(input, 2), get_number_text: (token) => token.text.substring(2) };
			else if (input.startsWith("0o"))
				return { length: TokenMatcher.match_octal(input, 2), get_number_text: (token) => token.text.substring(2) };
			else if (input.startsWith("0x"))
				return { length: TokenMatcher.match_hex(input, 2), get_number_text: (token) => token.text.substring(2) };
	...
	*/
	...
	pick_next() {
		while (this.index < this.text.length) {
			const part = this.text.substring(this.index);

			for (const type of this.token_types) {
				const result = type.match(part);

				if (result && result.length > 0) {
					this.current = new Token(type, part.substring(0, result.length), this.index, result);
